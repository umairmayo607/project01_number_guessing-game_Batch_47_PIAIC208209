...
async function playRound() {
  ...
  console.log(
    chalk.yellowBright(
      `Guess the number between ${minimum} and ${maximum}. You have ${maxAttempts} attempts.`
    )
  );
    ...
    const answer = await inquirer.prompt([
      {
        type: "input",
        name: "guess",
        message: chalk.bgMagentaBright`Attempt ${
          attempts + 1
        }: Enter your guess: `,
        validate: (value) => {
  ...
  if (playAgain.again === "Yes") {
    ...
    console.log(
      chalk.green(
        `Rounds played: ${rounds}, Total Attempts: ${totalAttempts}, Wins: ${wins}`
      )
    );
  }
}
